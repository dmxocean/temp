...
def main():
    ...
    # Get model paths
    model_paths = {
        "checkpoint_path": os.path.join(config_manager.get_model_dir("attention"), "checkpoint.pth"),
        "best_model_path": os.path.join(config_manager.get_model_dir("attention"), "best_model.pth")
    }
    ...
    history_path = os.path.join(config_manager.get_model_dir("attention"), "training_history.json")
    save_json(history, history_path)
    print(f"\nSaved training history to {history_path}")
    
    # Plot training history
    plot_path = os.path.join(config_manager.get_model_dir("attention"), "training_curves.png")
    ...
    results_path = os.path.join(config_manager.get_model_dir("attention"), "test_results.json")
    save_json(test_results, results_path)
    print(f"\nSaved test results to {results_path}")
    
    # Visualize sample captions
    print("\nGenerating sample captions...")
    viz_dir = os.path.join(config_manager.get_model_dir("attention"), "visualizations")
